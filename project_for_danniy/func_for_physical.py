...
def name(namee,idd):
    print(idd)
    try:
        s = df_name[df_name['#Name'] == str(namee).upper()]
        name_popularity = 0
        new_name = ''
        ...
        return df_name[(df_name['GROUP'] == 1)].sample().iloc[0]['#Name'].capitalize()


def surname(name,idd):
    print(idd)
    try:
        ...
        return df_fam[(df_fam['GROUP'] == 1)].sample().iloc[0]['#Формат: Фамилия'].capitalize()
def patronymic(name,idd):
    print(idd)
    try:
        ...
        return df_otch[(df_otch['GROUP'] == 1)].sample().iloc[0]['#Отчество'].capitalize()
