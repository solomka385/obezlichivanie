  ...
  #Обновление экрана
    ...
    def clicked(self):
        ...
        if 'email' in self.table_input.text():
            ...
            global_strok = ''
            self.w2.shor()
            df_email = pd.read_sql(("select physical_party.hid_party, email, surname, name, patronymic from physical_party join email on physical_party.hid_party = email.hid_party limit 30;"),cdi_stress)
          #  df_email['name'] = df_email.apply(lambda x: name(x.name,x.hid_party), axis = 1)
           ...
           # df_email['patronymic'] = df_email.apply(lambda x: patronymic(x.patronymic,x.hid_party), axis = 1)
            ...
            df_email = df_email.apply(patronymic,axis = 1)
