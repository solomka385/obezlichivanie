...
class SecondWindow(QWidget):
    ...
    def __init__(self, parent=None, flags=Qt.WindowFlags()):
        ...
        self.setWindowIcon(QtGui.QIcon('C://Users/alex1/Desktop/project_for_danniy/logo.jpg'))
        self.initUI()
        self.set_appear()  
  ...
  #Обновление экрана
    ...
    def clicked(self):
        ...
        if  'physical_party' in self.table_input.text():
            ...
            global_strok = ''
            self.w2.shor()
            global_strok = (tabulate( df[['hid_party','inn','surname','name','patronymic','birthdate','snils']], headers='keys', tablefmt='psql',showindex=False)) 
            df = df[['hid_party','inn','surname','name','patronymic','birthdate','snils']]
            self.w2.shor()
            global_strok = ''
            self.w2.shor()
            if 'name' in stolb:
        ...
        if 'email' in self.table_input.text():
            ...
            global_strok = ''
            self.w2.shor()
            df_email = pd.read_sql(("select physical_party.hid_party, email, surname, name from physical_party join email on physical_party.hid_party = email.hid_party limit 30;"),cdi_stress)
          #  df_email['name'] = df_email.apply(lambda x: name(x.name,x.hid_party), axis = 1)
           ...
           # df_email['patronymic'] = df_email.apply(lambda x: patronymic(x.patronymic,x.hid_party), axis = 1)
            df_email = df_email.apply(name,axis = 1)
            df_email = df_email.apply(surname,axis = 1)
            
            
            
            for index in range(len(df_email)):
                fio = str(df_email['name'].iloc[index]) + str(df_email['surname'].iloc[index])
       ...
       #     global_strok = df_email[['hid_party','email']].to_string()
        ...
        if 'phone' in self.table_input.text():
            ...
            global_strok = (tabulate(df[['hid_party','countrycode','citycode','telephone']], headers='keys', tablefmt='psql',showindex=False)) 
            self.w2.shor()
        if 'address' in self.table_input.text():
            pass
        cdi_stress.close()
