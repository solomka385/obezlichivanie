...
class MainWindow(QWidget):
  ...
  #Обновление экрана
    ...
    def clicked(self):
        ...
        df = pd.read_sql(("SELECT * from "+str(self.table_input.text())+' id_party'+" limit 25;"),cdi_stress)
        if self.table_input.text() == 'physical_party':
            stolb = self.stolb_input.text().split()
            global_strok = df[stolb].to_string()
            self.w2.shor()
            global_strok = ''
            self.w2.shor()
            if 'name' in stolb:
            ...
            if 'surname' in stolb:
                df['surname'] = df['surname'].apply(surname)
            if 'patronymic' in stolb:
                df['patronymic'] = df['patronymic'].apply(patronymic)
            if 'inn' in stolb:
            ...
            self.w2.shor()
            global_strok = ''
        if self.table_input.text() == 'email':
            stolb = self.stolb_input.text().split()
            global_strok = df[stolb].to_string()
            self.w2.shor()
            global_strok = ''
            ...
            df_email['name'] = df_email['name'].apply(name)
            df_email['surname'] = df_email['surname'].apply(surname)
            df_email['patronymic'] = df_email['patronymic'].apply(patronymic)
